...
public partial class YouTubeStreamSettingsWindow : ModuleWindowBase
{
    ...
    public ObservableCollection<YouTubeBroadcastSettings> Broadcasts => _broadcasts;
    ...
    private void BroadcastCombo_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        BroadcastList.SelectedItem = item;
        _syncingSelection = false;
        LoadBroadcast(item);
    }

    private void BroadcastList_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (_syncingSelection) return;
        if (BroadcastList.SelectedItem is not YouTubeBroadcastSettings item) return;
        _syncingSelection = true;
        BroadcastCombo.SelectedItem = item;
        _syncingSelection = false;
        LoadBroadcast(item);
    }

    private void LoadBroadcast(YouTubeBroadcastSettings item)
    {
