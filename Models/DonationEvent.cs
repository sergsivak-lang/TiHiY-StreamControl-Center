...
public sealed class DonationEvent
{
    ...
    public string EventIcon => IsTest ? "T" : IsReplay ? "↻" : Kind.Equals("SUBSCRIPTION", StringComparison.OrdinalIgnoreCase) ? "★" : "♥";
    public string KindLabel => IsTest ? "TEST" : IsReplay ? "ПОВТОР ALERT" : Kind.Equals("SUBSCRIPTION", StringComparison.OrdinalIgnoreCase) ? "ПІДПИСКА" : "ДОНАТ";
    public string EventSummary => Kind.Equals("SUBSCRIPTION", StringComparison.OrdinalIgnoreCase) ? Message : $"{DisplayAmount} • {Message}";
    public string StableId => string.IsNullOrWhiteSpace(ExternalId) ? $"{Source}:{Kind}:{Time.Ticks}:{User}:{Amount}:{Currency}" : ExternalId;
}
